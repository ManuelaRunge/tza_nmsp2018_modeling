...
ScenDat %>%
  dplyr::select(Strata, FutScen_label_noCM, CMincrease) %>%
  unique() %>%
...
ggsave("Fig_3.png", plot = pplot, path = 'figures', width = 14, height = 10, device = "png")
if (SAVEpdf)ggsave("Fig_3.pdf", plot = pplot, path = 'figures', width = 14, height = 10, device = "pdf")


fwrite(ScenDat, file.path('figures', 'figuredat', 'figure3_dat.csv'))
fwrite(baselineDat, file.path('figures', 'figuredat', 'baselineDat.csv'))

rm(pp1, pp4, pp2, pp5, pp3, ScenDat, baselineDat, pplot, AnalysisDat)